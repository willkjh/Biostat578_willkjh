...
res2 <- merge(gse[gse_gpl][grepl("HCV", summary) & grepl("yale.edu", contact), 
    j = list(title, gse, gpl)], gpl[grepl("Illumina", manufacturer), j = list(gpl, 
    manufacturer, description)], by = "gpl")
```
