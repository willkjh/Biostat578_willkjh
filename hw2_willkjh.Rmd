...
```
In the above, we obtain the 43 probesets as described for their heatmap. The p-values were not similar to what was done  in their output in S1 under the TwoWay pvalue. 
...
heatmap.2((heatmapFinal)[match(names(S1Table$DiffFC[order(S1Table$DiffFC)]), 
                    rownames(heatmapFinal)), ], 
          Rowv=NA, Colv=NA, scale="row", trace= "none")

fDataID <- fData(gds2use)[rownames(heatmapFinal),]
heatmapFinal.scaled <- t(scale(t(heatmapFinal), rowMeans(heatmapFinal), apply(heatmapFinal, 1, sd)))
heatmap.2(heatmapFinal.scaled[match(names(S1Table$DiffFC[order(S1Table$DiffFC)]),
                                    rownames(heatmapFinal)),],
          Rowv=NULL, Colv=NULL,
          dendrogram="none", 
